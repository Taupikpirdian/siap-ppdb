...
<div class="page-header row no-gutters py-4">
  <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
    <h3 class="page-title">Bukti Tuntas Penerimaan</h3>
  </div>
</div>
  ...
  <div class="col">
    <div class="card card-small mb-5">
      <div class="card-header border-bottom">
        ...
        {!! Form::open(['method'=>'GET','url'=>'searchbukti','role'=>'search'])  !!}
            <div class=" col-md-6 input-group mb-1 px-0 " >
            @if (Auth::user()->hasAnyRole('Create Bukti Tuntas'))
                <a title="Create" href="{{URL::to('/bukti/create')}}" class="button button-primary" style="margin:2px;"><i class="fa fa-plus-circle"></i></a>
            @endif 
                ...
                <a title="Create" href="{{URL::to('/bukti/index')}}" style="margin:2px;" class="button button-primary"><i class="fas fa-sync"></i></a>
          ...
          <div id="filter" class='col-md-12 px-0'>
            {!! Form::open(['method'=>'GET','url'=>'filterbukti','role'=>'filter'])  !!}          
        ...
        <table class="table table table-bordered table-striped table-hover table-condensed tfix mb-1" style="font-family: Arial; font-size: 13px">
          <thead class="bg-light">
            <tr>
              ...
              <th scope="col" class="border-0">Nama Penerima</th>
              <th scope="col" class="border-0">Asal Penerima</th>
              <th scope="col" class="border-0">Tanggal Penerima</th>
              <th scope="col" class="border-0" style="width:110px;">Jumlah Penerimaan</th>
          ...
          <tstatus>
            @foreach($bukti as $i=>$buktis)
          <tr>
             <td>{{ ($bukti->currentpage()-1) * $bukti->perpage() + $i + 1 }}</td>
             <td> {{ $buktis->kode_penerimaan }} </td>
             <td> {{ $buktis->nama_penerimaan }} </td>
             <td> {{ $buktis->asal_penerimaan }} </td>
             <td> {{ Carbon\Carbon::parse($buktis->created_at)->formatLocalized('%d %B %Y')}} </td>
             <td> Rp. {{ number_format($buktis->jumlah_penerimaan) }} </td>
             <td> Rp. {{ number_format($buktis->jumlah_penyaluran) }} </td>
             
             <td> @if($buktis->status_penyaluran == 1)
                    <span title="Tuntas" class="btn btn-success btn-sm"><i class="fas fa-check"></i></span>
                  @elseif($buktis->status_penyaluran == 2)
             ...
             <td>
             @if (Auth::user()->hasAnyRole('Delete Bukti Tuntas'))
              <form title="Delete" class="btn btn-danger btn-sm" id="delete_buktis{{$buktis->id}}" action='{{URL::action("admin\BuktiTuntasController@destroy",array($buktis->id))}}' method="POST">
                  <input type="hidden" name="_method" value="delete">
                  <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  <a href="#" onclick="document.getElementById('delete_buktis{{$buktis->id}}').submit();"><i class="fa fa-trash fa-xs" style="color: white"></i></a>
              </form>
            @endif
            <a title="Download Bukti Pertanggungjawaban" class="btn btn-primary btn-sm" target="_blank" href="{{URL::to('/files/file/'.$buktis->file)}}" ><i class="fa fa-download fa-xs "> </i></a>
            </td>          
          </tr>
         @endforeach
    </tstatus>
