          ...
          <tstatus>
		   @foreach($groups as $i=>$group)
		         ...
		         <td>
					...
					<a class="btn btn-info btn-sm" href='{{URL::action("admin\GroupController@show",array($group->id))}}'><i class="fa fa-eye fa-xs"></i></a>
					<form class="btn btn-danger btn-sm" onclick="return confirm('Apakah anda yakin untuk menghapus data ini?')" id="delete_group{{$group->id}}" action='{{URL::action("admin\GroupController@destroy",array($group->id))}}' method="POST">
