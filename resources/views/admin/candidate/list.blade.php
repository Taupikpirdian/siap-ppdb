  ...
  <div class="col">
    <div class="card card-small mb-5">
      <div class="card-header border-bottom">
		...
		{!! Form::open(['method'=>'GET','url'=>'/searchcandidate','role'=>'search'])  !!}
		<div class=" col-md-5 input-group mb-1 px-0">
			@if (Auth::user()->hasAnyRole('Create Calon'))
			<a href="{{URL::to('candidate/create')}}" style="margin:2px;" class="button button-primary"><i class="fa fa-plus-circle"></i></a>
			@endif
		...
		{!! Form::close() !!}
            ...
            {!! Form::open(['method'=>'GET','url'=>'filtercandidate','role'=>'filter'])  !!}
          ...
          <tstatus>
		   @foreach($candidate as $i=>$candidates)
             ...
             <td> {{Carbon\Carbon::parse($candidates->created_at)->formatLocalized('%d %B %Y')}} </td>
		         ...
		         <td>
                @if (Auth::user()->hasAnyRole('Aktif Calon') && $candidates->status == "Calon" && $candidates->payment_status == 2)
                <a title="Aktifkan Siswa" class="btn btn-success btn-sm" onclick="return confirm('Apakah anda yakin untuk mengaktifkan siswa ini?');" href='{{URL::action("admin\CandidateController@approve",array($candidates->id))}}'><i class="fa fa-check"></i></a>
                @endif

                @if (Auth::user()->hasAnyRole('Edit Calon') && $candidates->status_id == 0)
                  <a title="Edit" class="btn btn-warning btn-sm" href='{{URL::action("admin\CandidateController@edit",array($candidates->id))}}'><i class="fa fa-edit fa-xs" style="color: white"></i></a>
                @endif

                @if (Auth::user()->hasAnyRole('Bayar Calon') && $candidates->status_id == 0)
                  @if($candidates->status == "Calon" && $candidates->payment_status == 2)
                  @else
                    <a title="Bayar" class="btn btn-success btn-sm" href='{{URL::action("admin\CandidateController@pay",array($candidates->id))}}'><i class="far fa-money-bill-alt"></i></a>
                  @endif
                @endif
                
                @if (Auth::user()->hasAnyRole('Details Calon'))
                  <a title="Detail" class="btn btn-info btn-sm" href='{{URL::action("admin\CandidateController@show",array($candidates->id))}}'><i class="fas fa-eye fa-xs"></i></a>
                @endif
                <!-- <a title="Print Invoice" class="btn btn-warning btn-sm" target="_blank" href='{{URL::action("admin\CandidateController@invoice",array($candidates->id))}}'><i class="fa fa-print fa-xs" style="color: white"></i></a> -->
    				</td>	         
		     	</tr>
			   @endforeach
		</tstatus>
